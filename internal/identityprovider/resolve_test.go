...
type world struct {
	// claims maps one domain to the live active provider that holds it.
	claims map[string]string
	// linked names the live active providers the person is linked to, and
	// active names every live active provider of the tenant.
	linked []string
	active []string
	...
	// could run refuses instead of naming a provider.
	broken error
}
...
// TestCaseOneDomainClaimAnswers covers case 1. A domain a live active provider
// claims names the directory, and it outranks every case below it, including a
// person who holds a local password and an Identity Link of their own.
...
func TestDomainMatchIgnoresCaseAndSpace(t *testing.T) {
	r, _ := testResolver(t, world{claims: map[string]string{theDomain: tenantIdpID}})
...
// TestCaseTwoOneIdentityLinkAnswers covers case 2. The person holds one Identity
// Link with a provider that takes a typed password, so that directory proves the
...
// TestCaseTwoRefusesTwoIdentityLinks covers the first refusal. One person holds
// at most one account per provider, which the unique key enforces, so two links
...
// TestCaseFourSoleProviderAnswers covers case 4. Nobody holds this identifier,
...
// TestCaseFourRefusesTwoProviders covers the second refusal. A tenant that
...
func TestCaseFourSkipsAnAccountTheTenantHolds(t *testing.T) {
	...
	if ran.active != 0 {
		t.Fatalf("the providers of the tenant were counted for an account that exists: %+v", ran)
	}
}

// TestTenantWithNoDirectoryResolvesNothing covers the tenant every deployment
// starts as. No provider is registered, so every identifier answers the local
// password compare and the sign-in is what it was.
func TestTenantWithNoDirectoryResolvesNothing(t *testing.T) {
...
func TestABrokenReadRefuses(t *testing.T) {
	...
	for _, name := range []string{"domain", "link", "account"} {
		w := world{broken: broken, claims: map[string]string{theDomain: tenantIdpID}}
...
// error says which of the two refusals ran. Case 2 counts Identity Links and
// case 4 counts providers, so a refusal that named its case would say whether
...
func TestResolveAnswersTheLocalCompareWhenItRefuses(t *testing.T) {
	cases := map[string]world{
		"two identity links": {linked: []string{orgIdpID, tenantIdpID}},
		"two providers":      {active: []string{orgIdpID, tenantIdpID}},
	}
	userIDs := map[string]string{"two identity links": testUserID, "two providers": ""}
	...
	for name, w := range cases {
		...
		if idpID != "" {
			t.Errorf("%s: Resolve named %q, want the local password compare", name, idpID)
		}
	}
}

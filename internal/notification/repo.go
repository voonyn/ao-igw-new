...
func (r *Repository) FindSettings(ctx context.Context, tenantID string) (Settings, error) {
	r.log.Debug("read the delivery settings", logger.String("tenant_id", tenantID), logger.RequestID(ctx))
...
func (r *Repository) UpsertSettings(ctx context.Context, row Settings) error {
	r.log.Debug("write the delivery settings", logger.String("tenant_id", row.TenantID), logger.RequestID(ctx))
...
func (r *Repository) UpsertTemplate(ctx context.Context, row Template) error {
	r.log.Debug("write notification template",
		logger.String("tenant_id", row.TenantID), logger.String("org_id", row.OrgID), logger.String("template_key", row.Key),
	...
	r.log.Debug("wrote notification template",
		logger.String("tenant_id", row.TenantID), logger.String("org_id", row.OrgID), logger.String("template_key", row.Key),
		logger.RequestID(ctx))
	return nil
}
...
func (r *Repository) RemoveTemplate(ctx context.Context, tenantID, orgID, key string) error {
	r.log.Debug("delete notification template",
		logger.String("tenant_id", tenantID), logger.String("org_id", orgID), logger.String("template_key", key),
		logger.RequestID(ctx))
	...
	r.log.Debug("deleted notification template",
		logger.String("tenant_id", tenantID), logger.String("org_id", orgID), logger.String("template_key", key),
		logger.RequestID(ctx))
	return nil
}

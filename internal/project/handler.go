...
// Handler serves the projects of a tenant to the console. It binds the request,
...
// AdminRoutes mounts the project routes. The caller mounts the tenant
...
func AdminRoutes(router fiber.Router, h *Handler) {
	router.Get("/projects", middlewares.Paginate("created", "name", "state"), h.list)
	router.Get("/projects/:id", h.find)
	router.Post("/projects", h.create)
	router.Patch("/projects/:id", h.update)
	router.Delete("/projects/:id", h.remove)
}

/**
 * Server-only STATELESS custody of the portal session and the pending auth flow.
 *
 * There is no server-side store. Both the pending authorization-code flow
 * (state/verifier/nonce/returnTo) and the finished session's token set
 * (access/refresh/id) travel with the browser as **sealed** cookies: a compact
 * JWE (dir + A256GCM) the browser cannot read (httpOnly) or decrypt (no key),
 * so the tokens remain opaque outside this process — the same "browser never
 * holds a usable token" property the old in-memory store gave us, minus the
 * store. This is what makes the BFF survive `next dev` (which evaluates each
 * route as a separate module graph, so a module-level Map written by
 * /auth/login is NOT the one /auth/callback reads → the old invalid_state
 * redirect loop) and horizontal scale — with no Redis.
 *
 * Key: AO_PORTAL_COOKIE_SECRET (>= 32 chars). Unset in production is fatal
...
// distinct from login-ui/console-ui cookies so BFFs never collide on a shared host.
export const PORTAL_SESSION_COOKIE = "__Host-ao_psid"
export const PORTAL_FLOW_COOKIE = "__Host-ao_pflow"
...
// A round-trip through the gateway authorize + login-ui should finish well within this.
const FLOW_TTL = "10m"
...
function key(): Promise<Uint8Array> {
  ...
  const secret = process.env.AO_PORTAL_COOKIE_SECRET
  let material: string
  if (secret && secret.length >= 32) {
    material = secret
  } else if (process.env.NODE_ENV === "production") {
    throw new Error("AO_PORTAL_COOKIE_SECRET (>= 32 chars) is required outside development")
  } else {
    if (!warned) {
      warned = true
      console.warn("portal: AO_PORTAL_COOKIE_SECRET unset — using an INSECURE dev key; set it before production")
    }
    material = "dev-only-insecure-portal-cookie-key-00000"
  }
